  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="w-full pt-4 border-t border-white/10 flex flex-col gap-2">
          ...
          <div className="flex gap-2.5 flex-wrap">
            <input
              ...
              readOnly
              value={referralUrl}
              className="flex-1 min-w-[240px] px-3.5 py-2 rounded-xl bg-white/10 border border-white/20 text-xs font-mono text-white focus:outline-none"
            />
            <Button variant="lime" size="md" onClick={handleCopy}>
            ...
            >
              <Button variant="glass" size="md" className="text-white border-white/20 hover:bg-white/20">
      ...
      <Card className="p-0 overflow-hidden shadow-sm dark:shadow-dark-card">
        <div className="p-5 border-b border-slate-border dark:border-[#3A3A3D]">
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-50 dark:bg-[#1E1E21] border-b border-slate-border dark:border-[#3A3A3D] text-slate-muted dark:text-[#A1A1AA] uppercase text-[11px] font-bold tracking-wider select-none">
                <th className="py-3 px-4">Friend Nickname</th>
                <th className="py-3 px-4">Invited Date</th>
                ...
                <tr key={f.nickname} className="hover:bg-slate-50/70 dark:hover:bg-[#323236] transition-colors">
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="font-bold text-slate-dark dark:text-[#F5F5F5]">{f.nickname}</span>
                    </div>
                  </td>
                  <td className="py-3.5 px-4 font-mono text-slate-muted dark:text-[#71717A]">{f.date}</td>
                  <td className="py-3.5 px-4">
                    <Badge variant={f.status.includes('Completed') ? 'up' : 'neutral'} size="sm">
                      {f.status}
                    </Badge>
                  </td>
                  <td className="py-3.5 px-4 text-right font-mono font-bold text-lime-900 dark:text-lime">
                    {f.reward}
                  </td>
                </tr>
              ))}
